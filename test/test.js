...
describe('Tests for Gender detection from name', () => {
  ...
  it('should return male for some turkish male names', () => {
    const names = ['Ufuk', 'Ayberk', 'Levent', 'Sedat', 'Harun'];
    for (const name of names) {
      assert.strictEqual(getGender(name, 'tr'), 'male');
    }
  });
  it('should return female for some turkish female names', () => {
    const names = ['Gülşen', 'Pelin', 'Aylin', 'Ebru', 'Tuğçe'];
    for (const name of names) {
      assert.strictEqual(getGender(name, 'tr'), 'female');
    }
  });
  ...
  it('should return male for some names with no language', () => {
    const names = ['Dave', 'John', 'Giacomo', 'George', 'Luke', 'Amalrich'];
    for (const name of names) {
      assert.strictEqual(getGender(name), 'male');
    }
  });
  it('should return female for some names with no language', () => {
    const names = ['Anna', 'Jennifer', 'Lisa', 'Marta', 'Catherine', 'Ayşe'];
    for (const name of names) {
